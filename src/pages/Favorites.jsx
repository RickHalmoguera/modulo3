      ...
      {favorites.map((photo) => (
        <div key={photo.img} className="photo-card">
          ...
          <div className="photo-details">
           { isEditModeOn? <input type='text' placeholder={photo.description} onChange={(e)=> setNewDescription(e.currentTarget.value)} /> : <p>{photo.description}</p> }
            <button >
              {isEditModeOn ? <SaveIcon  onClick={()=> handleEdit(photo)}/> : <EditIcon onClick={()=> handleClick(photo)}/> }
            </button>
            <p>Date: {photo.date}</p>
